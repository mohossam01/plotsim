...
>
> **Aligns with:** `__version__ = 0.5.0` · post-M127a + M127b + M128 (fidelity rebaseline) + M129 (templates folder rename `configs/new/` → `configs/templates/`, public `list_templates`/`load_template` API) · commit `7d26b59` (HEAD pre-M129) · 2026-05-02
>
...
`plotsim/configs/templates/saas_template.{yaml,py}` (full reference) or
`plotsim/configs/templates/bare_minimum.yaml` (smallest working).
...
from plotsim import (
    ...
    # Template discovery (M129) — bundled config browse / load
...
input format (see `plotsim/configs/templates/saas_template.{yaml,py}` for the
...
`list_templates()` and `load_template(name)` (M129) are the discovery surface
over `plotsim/configs/templates/`. `list_templates() -> list[str]` returns the
bundled names (`bare_minimum`, `education`, `hr`, `marketing`, `retail`, `saas`),
sorted; `load_template(name) -> PlotsimConfig` resolves the name to its bundled
YAML and routes through `create_from_yaml`. Both use `importlib.resources` so
they work against installed wheels, not just an editable checkout. Raise
`ValueError` (with the available list) on unknown names.
