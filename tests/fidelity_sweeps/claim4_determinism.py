...
def _spawn_and_hash(cwd: Path, out_dir: Path,
                    seed_override: int | None = None) -> dict[str, str]:
    ...
    seed_line = (
        ...
        if seed_override is not None else ""
    )
...
def _cross_process_pair(*, varied_cwd: bool = False,
                         seed_override_b: int | None = None
                         ) -> dict[str, tuple[str, str]]:
    """Return {filename: (hash_proc_a, hash_proc_b)}."""
    with tempfile.TemporaryDirectory() as ta, tempfile.TemporaryDirectory() as tb, \
         tempfile.TemporaryDirectory() as cwd_a, tempfile.TemporaryDirectory() as cwd_b:
        ...
        hashes_b = _spawn_and_hash(
            cwd_b_path, out_b, seed_override=seed_override_b,
        )
...
def run_claim4(out_csv: Path = RESULT_CSV) -> int:
    ...
    sys.stderr.write(
        f"[claim4] wrote {len(df)} rows to {out_csv} in "
