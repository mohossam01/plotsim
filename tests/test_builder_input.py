...
def test_connection_string_wrong_token_count_rejected():
    with pytest.raises(ValidationError, match="three"):
        UserInput.model_validate(_minimal_input(connections=["engagement driven_by mrr extra"]))
...
def test_score_metric_works_without_range():
    MetricInput.model_validate({"name": "engagement", "type": "score", "polarity": "positive"})


def test_count_metric_works_without_range():
    MetricInput.model_validate({"name": "tickets", "type": "count", "polarity": "negative"})


def test_amount_metric_without_range_rejected():
    with pytest.raises(ValidationError, match="range"):
        MetricInput.model_validate({"name": "mrr", "type": "amount", "polarity": "positive"})


def test_index_metric_without_range_rejected():
    with pytest.raises(ValidationError, match="range"):
        MetricInput.model_validate({"name": "nps", "type": "index", "polarity": "positive"})
...
def test_two_metric_causal_cycle_detected():
    data = _minimal_input(
        metrics=[
            {"name": "a", "type": "score", "polarity": "positive", "follows": "b", "delay": 1},
            {"name": "b", "type": "score", "polarity": "positive", "follows": "a", "delay": 1},
        ]
    )
...
def test_three_metric_causal_cycle_detected():
    data = _minimal_input(
        metrics=[
            {"name": "a", "type": "score", "polarity": "positive", "follows": "b", "delay": 1},
            {"name": "b", "type": "score", "polarity": "positive", "follows": "c", "delay": 1},
            {"name": "c", "type": "score", "polarity": "positive", "follows": "a", "delay": 1},
        ]
    )
    with pytest.raises(ValidationError, match="cycle"):
        UserInput.model_validate(data)
...
def test_follows_chain_without_cycle_accepted():
    ...
    data = _minimal_input(
        metrics=[
            ...
            {"name": "b", "type": "score", "polarity": "positive", "follows": "a", "delay": 1},
            {"name": "c", "type": "score", "polarity": "positive", "follows": "b", "delay": 1},
        ]
    )
    UserInput.model_validate(data)
...
def test_baseline_orphan_metric_rejected():
    data = _minimal_input(
        segments=[
            {"name": "alpha", "count": 10, "archetype": "growth", "baseline": {"phantom": "high"}},
            {"name": "beta", "count": 10, "archetype": "decline"},
        ]
    )
...
def test_seasonal_with_long_window_no_warning():
    ...
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        UserInput.model_validate(data)
    assert not any("seasonal" in str(w.message) for w in caught)
...
def test_single_segment_emits_warning():
    ...
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        UserInput.model_validate(data)
    assert any("one segment" in str(w.message) for w in caught)


def test_mirrors_with_eight_metrics_emits_warning():
    metrics = [{"name": f"m{i}", "type": "score", "polarity": "positive"} for i in range(8)]
    data = _minimal_input(
        metrics=metrics,
    ...
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        UserInput.model_validate(data)
    assert any("mirrors" in str(w.message) or "PSD" in str(w.message) for w in caught)
