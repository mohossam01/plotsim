...
def _trivial_config(seed: int, *, configured_corr: float) -> PlotsimConfig:
    ...
    """
    archetype = Archetype(
        name="trivial_arch", label="trivial",
    ...
    metrics = [
        Metric(name="m_a", label="metric a", distribution="beta",
               params={"alpha": 2.0, "beta": 5.0}, polarity="positive",
               value_range={"min": 0.0, "max": 1.0}),
        Metric(name="m_b", label="metric b", distribution="beta",
               params={"alpha": 2.0, "beta": 5.0}, polarity="positive",
               value_range={"min": 0.0, "max": 1.0}),
    ]
    ...
    with warnings.catch_warnings():
        ...
        return PlotsimConfig(
            domain=Domain(name="trivial", description="trivial",
            ...
            tables=[
                ...
                Table(name="dim_user", type="dim", grain="per_entity",
                      primary_key="user_id",
                      columns=[
                ...
                Table(name="fct_metrics", type="fact",
                      ...
                      columns=[
                          ...
                          Column(name="user_id", dtype="id",
                                 source="fk:dim_user.user_id"),
                          Column(name="m_a_value", dtype="float",
                                 source="metric:m_a"),
                          Column(name="m_b_value", dtype="float",
                                 source="metric:m_b"),
                      ]),
            ],
            correlations=[CorrelationPair(metric_a="m_a", metric_b="m_b",
                                          coefficient=configured_corr)],
...
def _build_trivial(cell: dict) -> PlotsimConfig:
    return _trivial_config(seed=cell["seed"],
                           configured_corr=cell["configured"])
...
def _measure_trivial(cell: dict, tables: dict) -> dict:
    ...
    import numpy as np
    return {"observed": float(np.corrcoef(a, b)[0, 1]),
            "n_samples": int(len(a))}
...
def test_sweep_runner_writes_expected_csv_schema(tmp_path: Path) -> None:
    ...
    n = run_sweep(
        cells, _build_trivial, _measure_trivial, out_csv,
        flush_every=4, progress_label="trivial",
    )
    assert n == 10

    df = pd.read_csv(out_csv)
    assert len(df) == 10
    expected_cols = {"configured", "dist_a", "dist_b", "seed",
                     "n_samples", "observed"}
    assert set(df.columns) == expected_cols, (
        f"trivial sweep schema drift: got {set(df.columns)}, "
...
def test_sweep_analyzer_produces_markdown_table_on_trivial_sweep(
    ...
    out_csv = tmp_path / "trivial.csv"
    run_sweep(cells, _build_trivial, _measure_trivial, out_csv,
              flush_every=10, progress_label="trivial-analyze")
    ...
    assert "| dist_a | dist_b | configured |" in table, (
    ...
    for col in ["observed_median", "observed_iqr_lo",
                "observed_iqr_hi", "observed_n"]:
...
def test_render_markdown_table_handles_empty_and_nan() -> None:
    ...
    nan_df = pd.DataFrame([{"label": "x", "value": float("nan"),
                            "value_n": 0}])
    rendered = render_markdown_table(
        nan_df, columns=["label", "value", "value_n"],
    )
...
def test_claim1_correlation_subset_within_documented_tolerance(
    dist_a: str, dist_b: str, documented_tol: float,
) -> None:
    ...
    observed = _simulate_pair_pearson(
        dist_a, dist_b, n_other=0,
        coefficient=coef, n_entities=30, n_periods=12, seed=7000,
    )
...
def test_claim2_small_lag_recovers_at_output_level() -> None:
    ...
    cfg = _build_cfg(
        archetype_name="sigmoid", lagged_dist="poisson",
        lag=2, blend=1.0,
        n_entities=30, n_periods=60, seed=9000,
    )
    import numpy as np
...
def test_claim3_trajectory_envelope_reproduces_on_saas_subset() -> None:
    ...
    rows = _verify_template_seed("saas", seed_idx=0, cells_per_run=50,
                                 rng=sample_rng)
    devs = np.array(
        [r["deviation_in_sigma"] for r in rows
         if np.isfinite(r["deviation_in_sigma"])],
        dtype=float,
    )
    assert len(devs) > 50, (
...
def test_claim4_same_process_determinism_on_saas() -> None:
    ...
    assert not differences, (
        f"Claim 4 smoke regression: same-process determinism broken for: "
        f"{differences[:3]!r}"
    )
